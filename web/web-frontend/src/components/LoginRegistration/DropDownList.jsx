export const bloodGroupList = [
    { value: 'A+', label: 'A+' },
    { value: 'B+', label: 'B+' },
    { value: 'A-', label: 'A-' },
    { value: 'B-', label: 'B-' },
    { value: 'O+', label: 'O+' },
    { value: 'O-', label: 'O-' },
    { value: 'AB+', label: 'AB+' },
    { value: 'AB-', label: 'AB-' }
  ];


export const provinceList = [
    { value: 'koshi', label: 'Koshi Province' },
    { value: 'madhesh', label: 'Madhesh Province' },
    { value: 'bagmati', label: 'Bagmati Province' },
    { value: 'gandaki', label: 'Gandaki Province' },
    { value: 'lumbini', label: 'Lumbini Province' },
    { value: 'karnali', label: 'Karnali Province' },
    { value: 'sudurpaschim', label: 'Sudurpashchim Province' }
];
  



export const ProvinceDistrictList = [
    { 
      label: 'Koshi Province', 
      options: [
        { value: 'Bhojpur', label: 'Bhojpur' },
        { value: 'Dhankuta', label: 'Dhankuta' },
        { value: 'Ilam', label: 'Ilam' },
        { value: 'Jhapa', label: 'Jhapa' },
        { value: 'Khotang', label: 'Khotang' },
        { value: 'Morang', label: 'Morang' },
        { value: 'Okhaldhunga', label: 'Okhaldhunga' },
        { value: 'Panchthar', label: 'Panchthar' },
        { value: 'Sankhuwasabha', label: 'Sankhuwasabha' },
        { value: 'Solukhumbu', label: 'Solukhumbu' },
        { value: 'Sunsari', label: 'Sunsari' },
        { value: 'Taplejung', label: 'Taplejung' },
        { value: 'Terhathum', label: 'Terhathum' },
        { value: 'Udayapur', label: 'Udayapur' }
      ]
    },
    { 
      label: 'Madhesh Province', 
      options: [
        { value: 'Bara', label: 'Bara' },
        { value: 'Dhanusha', label: 'Dhanusha' },
        { value: 'Mahottari', label: 'Mahottari' },
        { value: 'Parsa', label: 'Parsa' },
        { value: 'Rautahat', label: 'Rautahat' },
        { value: 'Saptari', label: 'Saptari' },
        { value: 'Sarlahi', label: 'Sarlahi' },
        { value: 'Siraha', label: 'Siraha' }
      ]
    },
    { 
      label: 'Bagmati Province', 
      options: [
        { value: 'Bhaktapur', label: 'Bhaktapur' },
        { value: 'Chitwan', label: 'Chitwan' },
        { value: 'Dhading', label: 'Dhading' },
        { value: 'Dolakha', label: 'Dolakha' },
        { value: 'Kathmandu', label: 'Kathmandu' },
        { value: 'Kavrepalanchok', label: 'Kavrepalanchok' },
        { value: 'Lalitpur', label: 'Lalitpur' },
        { value: 'Makwanpur', label: 'Makwanpur' },
        { value: 'Nuwakot', label: 'Nuwakot' },
        { value: 'Ramechhap', label: 'Ramechhap' },
        { value: 'Rasuwa', label: 'Rasuwa' },
        { value: 'Sindhuli', label: 'Sindhuli' },
        { value: 'Sindhupalchok', label: 'Sindhupalchok' }
      ]
    },
    { 
      label: 'Gandaki Province', 
      options: [
        { value: 'Baglung', label: 'Baglung' },
        { value: 'Gorkha', label: 'Gorkha' },
        { value: 'Kaski', label: 'Kaski' },
        { value: 'Lamjung', label: 'Lamjung' },
        { value: 'Manang', label: 'Manang' },
        { value: 'Mustang', label: 'Mustang' },
        { value: 'Myagdi', label: 'Myagdi' },
        { value: 'Nawalpur', label: 'Nawalpur' },
        { value: 'Parbat', label: 'Parbat' },
        { value: 'Syangja', label: 'Syangja' },
        { value: 'Tanahun', label: 'Tanahun' }
      ]
    },
    { 
      label: 'Lumbini Province', 
      options: [
        { value: 'Arghakhanchi', label: 'Arghakhanchi' },
        { value: 'Banke', label: 'Banke' },
        { value: 'Bardiya', label: 'Bardiya' },
        { value: 'Dang', label: 'Dang' },
        { value: 'Gulmi', label: 'Gulmi' },
        { value: 'Kapilvastu', label: 'Kapilvastu' },
        { value: 'Parasi', label: 'Parasi' },
        { value: 'Palpa', label: 'Palpa' },
        { value: 'Pyuthan', label: 'Pyuthan' },
        { value: 'Rolpa', label: 'Rolpa' },
        { value: 'Rukum (East)', label: 'Rukum (East)' },
        { value: 'Rupandehi', label: 'Rupandehi' }
      ]
    },
    { 
      label: 'Karnali Province', 
      options: [
        { value: 'Dailekh', label: 'Dailekh' },
        { value: 'Dolpa', label: 'Dolpa' },
        { value: 'Humla', label: 'Humla' },
        { value: 'Jajarkot', label: 'Jajarkot' },
        { value: 'Jumla', label: 'Jumla' },
        { value: 'Kalikot', label: 'Kalikot' },
        { value: 'Mugu', label: 'Mugu' },
        { value: 'Rukum (West)', label: 'Rukum (West)' },
        { value: 'Salyan', label: 'Salyan' },
        { value: 'Surkhet', label: 'Surkhet' }
      ]
    },
    { 
      label: 'Sudurpashchim Province', 
      options: [
        { value: 'Achham', label: 'Achham' },
        { value: 'Baitadi', label: 'Baitadi' },
        { value: 'Bajhang', label: 'Bajhang' },
        { value: 'Bajura', label: 'Bajura' },
        { value: 'Dadeldhura', label: 'Dadeldhura' },
        { value: 'Darchula', label: 'Darchula' },
        { value: 'Doti', label: 'Doti' },
        { value: 'Kailali', label: 'Kailali' },
        { value: 'Kanchanpur', label: 'Kanchanpur' }
      ]
    }
  ];

  export const venueList = [
    { value: 'Community Hall', label: 'Community Hall' },
    { value: 'City Hospital Conference Room', label: 'City Hospital Conference Room' },
    { value: 'University Auditorium', label: 'University Auditorium' },
    { value: 'Shopping Mall Atrium', label: 'Shopping Mall Atrium' },
    { value: 'Town Square', label: 'Town Square' },
    { value: 'Local Health Post', label: 'Local Health Post' },
    { value: 'Sports Complex', label: 'Sports Complex' },
    { value: 'School Gymnasium', label: 'School Gymnasium' },
    { value: 'Corporate Office Lobby', label: 'Corporate Office Lobby' },
    { value: 'District Red Cross Office', label: 'District Red Cross Office' },
    { value: 'Religious Center Hall', label: 'Religious Center Hall' },
    { value: 'Police Station Community Room', label: 'Police Station Community Room' },
    { value: 'Fire Station Training Room', label: 'Fire Station Training Room' },
    { value: 'Neighborhood Park Pavilion', label: 'Neighborhood Park Pavilion' },
    { value: 'Military Base Hall', label: 'Military Base Hall' },
    { value: 'Airport Terminal Lobby', label: 'Airport Terminal Lobby' },
    { value: 'Local Library Meeting Room', label: 'Local Library Meeting Room' },
    { value: 'Bus Station Waiting Area', label: 'Bus Station Waiting Area' },
    { value: 'Train Station Platform', label: 'Train Station Platform' },
    { value: 'Cultural Center', label: 'Cultural Center' },
    { value: 'Conference Center', label: 'Conference Center' },
    { value: 'Hotel Ballroom', label: 'Hotel Ballroom' },
    { value: 'City Council Hall', label: 'City Council Hall' },
    { value: 'Marquee in Open Field', label: 'Marquee in Open Field' },
  ];
  
  export const hospitalBloodDataList = [
    { 
      name: 'BPKIHS Dharan', 
      bloodType: [
        { type: 'A+', pint: 45 },
        { type: 'A-', pint: 20 },
        { type: 'B+', pint: 60 },
        { type: 'B-', pint: 15 },
        { type: 'AB+', pint: 30 },
        { type: 'AB-', pint: 10 },
        { type: 'O+', pint: 50 },
        { type: 'O-', pint: 25 },
      ]
    },
    { 
      name: 'Koshi Zonal Hospital', 
      bloodType: [
        { type: 'A+', pint: 40 },
        { type: 'A-', pint: 18 },
        { type: 'B+', pint: 55 },
        { type: 'B-', pint: 12 },
        { type: 'AB+', pint: 25 },
        { type: 'AB-', pint: 8 },
        { type: 'O+', pint: 48 },
        { type: 'O-', pint: 22 },
      ]
    },
    { 
      name: 'Mechi Hospital', 
      bloodType: [
        { type: 'A+', pint: 35 },
        { type: 'A-', pint: 22 },
        { type: 'B+', pint: 50 },
        { type: 'B-', pint: 20 },
        { type: 'AB+', pint: 28 },
        { type: 'AB-', pint: 12 },
        { type: 'O+', pint: 42 },
        { type: 'O-', pint: 30 },
      ]
    },
    { 
      name: 'Udayapur District Hospital', 
      bloodType: [
        { type: 'A+', pint: 38 },
        { type: 'A-', pint: 19 },
        { type: 'B+', pint: 45 },
        { type: 'B-', pint: 14 },
        { type: 'AB+', pint: 26 },
        { type: 'AB-', pint: 9 },
        { type: 'O+', pint: 40 },
        { type: 'O-', pint: 18 },
      ]
    },
    { 
      name: 'Taplejung District Hospital', 
      bloodType: [
        { type: 'A+', pint: 28 },
        { type: 'A-', pint: 16 },
        { type: 'B+', pint: 38 },
        { type: 'B-', pint: 10 },
        { type: 'AB+', pint: 22 },
        { type: 'AB-', pint: 5 },
        { type: 'O+', pint: 32 },
        { type: 'O-', pint: 14 },
      ]
    },
    { 
      name: 'Janakpur Zonal Hospital', 
      bloodType: [
        { type: 'A+', pint: 42 },
        { type: 'A-', pint: 20 },
        { type: 'B+', pint: 48 },
        { type: 'B-', pint: 15 },
        { type: 'AB+', pint: 30 },
        { type: 'AB-', pint: 12 },
        { type: 'O+', pint: 50 },
        { type: 'O-', pint: 24 },
      ]
    },
    { 
      name: 'Gaur District Hospital', 
      bloodType: [
        { type: 'A+', pint: 34 },
        { type: 'A-', pint: 16 },
        { type: 'B+', pint: 40 },
        { type: 'B-', pint: 12 },
        { type: 'AB+', pint: 22 },
        { type: 'AB-', pint: 8 },
        { type: 'O+', pint: 38 },
        { type: 'O-', pint: 14 },
      ]
    },
    { 
      name: 'Rajbiraj Hospital', 
      bloodType: [
        { type: 'A+', pint: 30 },
        { type: 'A-', pint: 14 },
        { type: 'B+', pint: 35 },
        { type: 'B-', pint: 10 },
        { type: 'AB+', pint: 20 },
        { type: 'AB-', pint: 7 },
        { type: 'O+', pint: 32 },
        { type: 'O-', pint: 12 },
      ]
    },
    { 
      name: 'Birgunj Blood Bank', 
      bloodType: [
        { type: 'A+', pint: 45 },
        { type: 'A-', pint: 18 },
        { type: 'B+', pint: 55 },
        { type: 'B-', pint: 14 },
        { type: 'AB+', pint: 28 },
        { type: 'AB-', pint: 10 },
        { type: 'O+', pint: 50 },
        { type: 'O-', pint: 22 },
      ]
    },
    { 
      name: 'Patan Hospital', 
      bloodType: [
        { type: 'A+', pint: 50 },
        { type: 'A-', pint: 22 },
        { type: 'B+', pint: 65 },
        { type: 'B-', pint: 18 },
        { type: 'AB+', pint: 35 },
        { type: 'AB-', pint: 12 },
        { type: 'O+', pint: 60 },
        { type: 'O-', pint: 28 },
      ]
    },
    { 
      name: 'Tribhuvan University Teaching Hospital', 
      bloodType: [
        { type: 'A+', pint: 52 },
        { type: 'A-', pint: 24 },
        { type: 'B+', pint: 62 },
        { type: 'B-', pint: 20 },
        { type: 'AB+', pint: 38 },
        { type: 'AB-', pint: 15 },
        { type: 'O+', pint: 58 },
        { type: 'O-', pint: 30 },
      ]
    },
    { 
      name: 'Bhaktapur Cancer Hospital', 
      bloodType: [
        { type: 'A+', pint: 35 },
        { type: 'A-', pint: 16 },
        { type: 'B+', pint: 40 },
        { type: 'B-', pint: 10 },
        { type: 'AB+', pint: 25 },
        { type: 'AB-', pint: 8 },
        { type: 'O+', pint: 38 },
        { type: 'O-', pint: 14 },
      ]
    },
    { 
      name: 'HAMS Hospital', 
      bloodType: [
        { type: 'A+', pint: 42 },
        { type: 'A-', pint: 18 },
        { type: 'B+', pint: 50 },
        { type: 'B-', pint: 14 },
        { type: 'AB+', pint: 30 },
        { type: 'AB-', pint: 12 },
        { type: 'O+', pint: 48 },
        { type: 'O-', pint: 20 },
      ]
    },
    { 
      name: 'Chitwan Medical College', 
      bloodType: [
        { type: 'A+', pint: 60 },
        { type: 'A-', pint: 25 },
        { type: 'B+', pint: 70 },
        { type: 'B-', pint: 20 },
        { type: 'AB+', pint: 40 },
        { type: 'AB-', pint: 15 },
        { type: 'O+', pint: 65 },
        { type: 'O-', pint: 30 },
      ]
    },
    { 
      name: 'Manipal Teaching Hospital', 
      bloodType: [
        { type: 'A+', pint: 46 },
        { type: 'A-', pint: 18 },
        { type: 'B+', pint: 58 },
        { type: 'B-', pint: 14 },
        { type: 'AB+', pint: 32 },
        { type: 'AB-', pint: 10 },
        { type: 'O+', pint: 54 },
        { type: 'O-', pint: 24 },
      ]
    },
    { 
      name: 'Gandaki Medical College', 
      bloodType: [
        { type: 'A+', pint: 50 },
        { type: 'A-', pint: 20 },
        { type: 'B+', pint: 62 },
        { type: 'B-', pint: 16 },
        { type: 'AB+', pint: 35 },
        { type: 'AB-', pint: 12 },
        { type: 'O+', pint: 58 },
        { type: 'O-', pint: 26 },
      ]
    },
    { 
      name: 'Pokhara Regional Hospital', 
      bloodType: [
        { type: 'A+', pint: 42 },
        { type: 'A-', pint: 18 },
        { type: 'B+', pint: 54 },
        { type: 'B-', pint: 12 },
        { type: 'AB+', pint: 30 },
        { type: 'AB-', pint: 9 },
        { type: 'O+', pint: 50 },
        { type: 'O-', pint: 22 },
      ]
    },
    { 
      name: 'Bheri Zonal Hospital', 
      bloodType: [
        { type: 'A+', pint: 36 },
        { type: 'A-', pint: 15 },
        { type: 'B+', pint: 48 },
        { type: 'B-', pint: 10 },
        { type: 'AB+', pint: 28 },
        { type: 'AB-', pint: 8 },
        { type: 'O+', pint: 44 },
        { type: 'O-', pint: 18 },
      ]
    },
    { 
      name: 'Nepalgunj Medical College', 
      bloodType: [
        { type: 'A+', pint: 40 },
        { type: 'A-', pint: 16 },
        { type: 'B+', pint: 52 },
        { type: 'B-', pint: 14 },
        { type: 'AB+', pint: 30 },
        { type: 'AB-', pint: 10 },
        { type: 'O+', pint: 48 },
        { type: 'O-', pint: 20 },
      ]
    },
    { 
      name: 'Seti Zonal Hospital', 
      bloodType: [
        { type: 'A+', pint: 38 },
        { type: 'A-', pint: 14 },
        { type: 'B+', pint: 50 },
        { type: 'B-', pint: 12 },
        { type: 'AB+', pint: 28 },
        { type: 'AB-', pint: 9 },
        { type: 'O+', pint: 46 },
        { type: 'O-', pint: 18 },
      ]
    },
    { 
      name: 'Dhangadhi District Hospital', 
      bloodType: [
        { type: 'A+', pint: 32 },
        { type: 'A-', pint: 12 },
        { type: 'B+', pint: 40 },
        { type: 'B-', pint: 10 },
        { type: 'AB+', pint: 22 },
        { type: 'AB-', pint: 7 },
        { type: 'O+', pint: 36 },
        { type: 'O-', pint: 14 },
      ]
    }
  ];

  export const provinceHospitalList = [
    { 
      label: 'Koshi Province', 
      options: [
        { value: 'BPKIHS Dharan', label: 'BPKIHS Dharan' },
        { value: 'Koshi Zonal Hospital', label: 'Koshi Zonal Hospital' },
        { value: 'Mechi Hospital', label: 'Mechi Hospital' },
        { value: 'Udayapur District Hospital', label: 'Udayapur District Hospital' },
        { value: 'Taplejung District Hospital', label: 'Taplejung District Hospital' },
      ]
    },
    { 
      label: 'Madhesh Province', 
      options: [
        { value: 'Janakpur Zonal Hospital', label: 'Janakpur Zonal Hospital' },
        { value: 'Gaur District Hospital', label: 'Gaur District Hospital' },
        { value: 'Rajbiraj Hospital', label: 'Rajbiraj Hospital' },
        { value: 'Birgunj Blood Bank', label: 'Birgunj Blood Bank' },
      ]
    },
    { 
      label: 'Bagmati Province', 
      options: [
        { value: 'Patan Hospital', label: 'Patan Hospital' },
        { value: 'Tribhuvan University Teaching Hospital', label: 'Tribhuvan University Teaching Hospital' },
        { value: 'Bhaktapur Cancer Hospital', label: 'Bhaktapur Cancer Hospital' },
        { value: 'HAMS Hospital', label: 'HAMS Hospital' },
        { value: 'Chitwan Medical College', label: 'Chitwan Medical College' },
      ]
    },
    { 
      label: 'Gandaki Province', 
      options: [
        { value: 'Manipal Teaching Hospital', label: 'Manipal Teaching Hospital' },
        { value: 'Gandaki Medical College', label: 'Gandaki Medical College' },
        { value: 'Pokhara Regional Hospital', label: 'Pokhara Regional Hospital' },
        { value: 'Myagdi District Hospital', label: 'Myagdi District Hospital' },
      ]
    },
    { 
      label: 'Lumbini Province', 
      options: [
        { value: 'Bhairahawa Medical College', label: 'Bhairahawa Medical College' },
        { value: 'Rapti Sub-Regional Hospital', label: 'Rapti Sub-Regional Hospital' },
        { value: 'Palpa Mission Hospital', label: 'Palpa Mission Hospital' },
        { value: 'Dang District Hospital', label: 'Dang District Hospital' },
      ]
    },
    { 
      label: 'Karnali Province', 
      options: [
        { value: 'Karnali Provincial Hospital', label: 'Karnali Provincial Hospital' },
        { value: 'Jumla District Hospital', label: 'Jumla District Hospital' },
        { value: 'Mugu District Hospital', label: 'Mugu District Hospital' },
        { value: 'Dolpa District Hospital', label: 'Dolpa District Hospital' },
      ]
    },
    { 
      label: 'Sudurpashchim Province', 
      options: [
        { value: 'Seti Zonal Hospital', label: 'Seti Zonal Hospital' },
        { value: 'Dhangadhi Blood Bank', label: 'Dhangadhi Blood Bank' },
        { value: 'Dadeldhura Hospital', label: 'Dadeldhura Hospital' },
        { value: 'Bajura District Hospital', label: 'Bajura District Hospital' },
      ]
    }
  ];


  export const timeShiftOptions = [
    { value: "morning", label: "Morning (9:00 AM - 12:00 PM)" },
    { value: "afternoon", label: "Afternoon (12:00 PM - 3:00 PM)" },
    { value: "evening", label: "Evening (3:00 PM - 6:00 PM)" },
  ];
  
  
  